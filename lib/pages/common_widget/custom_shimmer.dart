...
//? Widget usato per i caricamenti !!!
...
//? Shimmer usato per mostrare un caricamento con una lista di elementi
class ShimmerListView extends StatelessWidget {
  const ShimmerListView({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) => Shimmer.fromColors(
        baseColor: MainColor.primaryColor,
        highlightColor: MainColor.secondaryColor,
        child: ListView.builder(
          itemCount: 4,
          itemBuilder: (_, __) => _buildEmptyElement(context),
          padding: const EdgeInsets.all(5.0),
        ),
      );

  Widget _buildEmptyElement(BuildContext context) => Container(
        height: context.read<SettingsProvider>().config.safeBlockVertical * 12,
        padding: const EdgeInsets.all(8.0),
        margin: EdgeInsets.only(
            bottom:
                context.read<SettingsProvider>().config.safeBlockVertical * 4),
  ...
  Widget _padd() => const SizedBox(height: 4.0);
}
